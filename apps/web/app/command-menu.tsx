...
function CommandOptions() {
  const { state } = useCommand<Action>();

  const groupedActions = state.filtered.filter((action) => action.group);
  const ungroupedActions = state.filtered.filter((action) => !action.group);

  const visibleGroups = Array.from(
    new Set(groupedActions.map((a) => a.group))
  ).filter(Boolean);
  ...
  return (
    <>
            ...
            >
              ...
              {typedAction.shortcut && (
                <span className="command-option-shortcut">
                  {typedAction.shortcut.map((key: string, index: number) => (
                    <kbd key={index} className="command-shortcut-key">
                      {key}
                    </kbd>
                  ))}
                </span>
      ...
      {visibleGroups.map((group) => {
        const groupActions = groupedActions.filter(
        ...
        return (
          <div key={group}>
            <CommandGroup
              heading={<div className="command-group-heading">{group}</div>}
            >
...
export function CommandMenu({
  ...
  const globalMenuOpen =
    externalOpen !== undefined ? externalOpen : internalMenuOpen;
  const setGlobalMenuOpen = onOpenChange || setInternalMenuOpen;
  const [isClosing, setIsClosing] = useState(false);
  const overlayRef = useRef<HTMLDivElement>(null);
  const dialogRef = useRef<HTMLDivElement>(null);
  const commandRef = useRef<CommandRef<Action>>(null);
  ...
  const triggerSubmenuTransition = useCallback(() => {
    if (dialogRef.current) {
      dialogRef.current.classList.add("transitioning");
      setTimeout(() => {
        if (dialogRef.current) {
          dialogRef.current.classList.remove("transitioning");
        }
      }, 100);
    }
  }, []);
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      } else if (e.key === "Escape" && globalMenuOpen && !isClosing) {
        e.preventDefault();
        handleClose();
      }
    };
  ...
  useEffect(() => {
    ...
    let unsubSubmenu: (() => void) | undefined;
    let unsubBack: (() => void) | undefined;

    const timer = setTimeout(() => {
      const commandRefCurrent = commandRef.current;
      if (!commandRefCurrent?.command) return;

      const handleSubmenuTransition = () => {
        triggerSubmenuTransition();
      };

      unsubSubmenu = commandRefCurrent.command.on(
        "submenu",
        handleSubmenuTransition
      );
      unsubBack = commandRefCurrent.command.on("back", handleSubmenuTransition);
    }, 100);

    return () => {
      clearTimeout(timer);
      if (unsubSubmenu) unsubSubmenu();
      if (unsubBack) unsubBack();
    };
  }, [globalMenuOpen, triggerSubmenuTransition]);
          ...
          >
            <Command
              ...
              onSelect={(option) => {
                ...
                const action = findAction(actions, option.id!);
                if (action?.action) {
                  action.action();
            ...
            >
              ...
              <CommandBreadcrumbs className="command-breadcrumbs" />
